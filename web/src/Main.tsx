...
export default function Main() {
  ...
  useEffect(() => {
    const fetchVideo = async () => {
      try {
        ...
        if (docSnap.exists()) {
          setFeaturedVideo(docSnap.data() as any);
        }
  ...
  const handleDeleteDeal = async (id: string) => {
    if(window.confirm("Delete this deal permanently?")) await deleteDoc(doc(db, 'deals', id));
  };
            ...
            <form onSubmit={handleAddDeal} className="space-y-4 bg-slate-700 p-4 rounded-lg">
              <div className="flex flex-col md:flex-row gap-4">
                <div className="flex-1">
                  ...
                  <input className="w-full p-2 rounded bg-slate-900 border border-slate-600 text-white" placeholder="Flex 24V Kit..." value={adminTitle} onChange={e => setAdminTitle(e.target.value)} />
                </div>
                <div className="flex-1">
                  <label className="block text-xs text-gray-400 mb-1">Affiliate URL</label>
                  <input className="w-full p-2 rounded bg-slate-900 border border-slate-600 text-white" placeholder="https://..." value={adminUrl} onChange={e => setAdminUrl(e.target.value)} />
                </div>
              </div>
              <div className="flex flex-col md:flex-row gap-4">
                <div className="w-32">
                  <label className="block text-xs text-gray-400 mb-1">Sale Price</label>
                  <input className="w-full p-2 rounded bg-slate-900 border border-slate-600 text-white" type="number" value={adminPrice} onChange={e => setAdminPrice(e.target.value)} />
                </div>
                <div className="w-32">
                  <label className="block text-xs text-gray-400 mb-1">Orig. Price</label>
                  <input className="w-full p-2 rounded bg-slate-900 border border-slate-600 text-white" type="number" value={adminOrigPrice} onChange={e => setAdminOrigPrice(e.target.value)} />
                </div>
                <div className="flex-1">
                  <label className="block text-xs text-gray-400 mb-1">Image URL</label>
                  <input className="w-full p-2 rounded bg-slate-900 border border-slate-600 text-white" placeholder="https://image.lowes.com/..." value={adminImage} onChange={e => setAdminImage(e.target.value)} />
                </div>
              </div>
              <div className="flex flex-col md:flex-row gap-4 items-end">
                <div className="w-full md:w-1/3">
                  <label className="block text-xs text-gray-400 mb-1">Store</label>
                  <select className="w-full p-2 rounded bg-slate-900 border border-slate-600 text-white" value={adminStore} onChange={e => setAdminStore(e.target.value)}>
                    {sortedAdminStores.map(([name, code]) => (
                ...
                <div className="w-full md:w-1/3">
                  ...
                  <select className="w-full p-2 rounded bg-slate-900 border border-slate-600 text-white" value={adminType} onChange={e => setAdminType(e.target.value)}>
                    <option value="Glitch">🔥 Glitch (Pinned Top)</option>
                    <option value="Sale">🏷️ Regular Sale</option>
                    <option value="BOGO">🎁 BOGO</option>
                  </select>
                </div>
                <button type="submit" className="w-full md:w-1/3 bg-green-600 hover:bg-green-500 text-white font-bold py-2 px-6 rounded flex items-center justify-center gap-2">
                  <PlusCircle className="w-4 h-4" /> Post Deal
                </button>
              </div>
            </form>
          </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {glitchDeals.map(deal => (
                  <div key={deal.id} className="bg-white border-2 border-red-500 rounded-xl p-4 flex items-center gap-4 shadow-lg hover:shadow-xl transition-all group relative overflow-hidden">
                    ...
                    {isAdmin && (
                      <button onClick={(e) => { e.preventDefault(); handleDeleteDeal(deal.id); }} className="absolute bottom-2 right-2 bg-slate-800 text-white p-2 rounded hover:bg-red-600 z-50"><Trash2 className="w-4 h-4" /></button>
                    )}
